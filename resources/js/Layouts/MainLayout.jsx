...
const MainLayout = () => {
    ...
    const navigation = [
        ...
        { name: 'Facturación', href: '/accounting/invoices', icon: FileText },
        ...
        <div className="flex h-screen bg-gray-100">
            ...
            <div className="w-64 flex-shrink-0 bg-blue-600 flex flex-col">
                ...
                <nav className="flex-1 px-2 py-4 space-y-1">
                    {(() => {
                        // Find the navigation item with the longest href that matches the current path
                        const activeItem = navigation.reduce((best, item) => {
                            if (location.pathname.startsWith(item.href) && item.href.length > (best?.href.length || 0)) {
                                return item;
                            }
                            return best;
                        }, null);
